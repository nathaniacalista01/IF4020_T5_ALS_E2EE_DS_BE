...
export const socketController = (io: Server, socket: Socket) => {
  ...
  socket.on("sendRealTimeMessage", async (payload: SendMessagePayload) => {
    const { roomId, senderId, receiverId, message, isSigned, signature, ecegVal } = payload;
    ...
    const response = await messageService.addMessage(senderId, receiverId, Number(roomId), message, isSigned, ecegVal, signature);
