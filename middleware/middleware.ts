...
import { adjustText, makeBlocksArrayToString, makeStringToBlocksArray } from "../services/utils/crypt/process";
...
export const decryptionMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!clientKeys.has(clientPort)) {
    res.status(401).send(JSON.stringify({ error: "Unauthorized, need to initialize handshake" }));
    return;
  }
  ...
  try {
    ...
    const decryptedBody = decryptECB(makeStringToBlocksArray(encryptedData, false), newkey)
...
export const encryptionMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.send = function (data) {
    const clientPort = req.connection.localPort;
    if (!clientKeys.has(clientPort)) {
      return originalSend.call(this, JSON.stringify({ error: "Unauthorized, need to initialize handshake" }));
    }
    ...
    try {
      ...
      const encryptedData = encryptECB(makeStringToBlocksArray(textAdjusted, false), keyBlocks);
      const encryptedString = makeBlocksArrayToString(encryptedData);

      return originalSend.call(this, JSON.stringify({ encrypted: encryptedString }));
    } catch (error) {
